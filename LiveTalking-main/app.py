...
from companion_llm import llm_response, companion
...
async def switch_character(request):
    ...
    try:
        params = await request.json()
        character = params.get('character', 'xiaonuan')
        avatar_id = params.get('avatar_id', 'wav2lip256_avatar1')
        sessionid = params.get('sessionid', 0)
        
        # 切换角色配置
        from companion_llm import set_character, CHARACTERS
        result = set_character(character)

        if result != "ok":
            return web.Response(
                content_type="application/json",
                text=json.dumps({"code": -1, "msg": "角色不存在"})
            )
        
        # 重新加载 avatar
        char_config = CHARACTERS.get(character, {})
        new_avatar_id = char_config.get('avatar', avatar_id)
        
        # 检查 opt 是否已定义
        if opt is None:
            return web.Response(
                content_type="application/json",
                text=json.dumps({"code": -1, "msg": "服务尚未初始化"})
            )
        
        if opt.model == 'wav2lip':
            from lipreal import load_avatar
            avatar = load_avatar(new_avatar_id)
        elif opt.model == 'musetalk':
            from musereal import load_avatar
            avatar = load_avatar(new_avatar_id)
        
        # 更新现有 session 的 avatar
        if sessionid in nerfreals and nerfreals[sessionid]:
            nerfreals[sessionid].avatar = avatar
        
        logger.info(f"Session {sessionid} 切换到角色: {character}, avatar: {new_avatar_id}")
        return web.Response(
            content_type="application/json",
            text=json.dumps({"code": 0, "msg": f"已切换到{character}", "avatar": new_avatar_id})
        )
    except Exception as e:
        logger.exception('切换角色失败:')
        return web.Response(
            content_type="application/json",
...
if __name__ == '__main__':
    ...
    appasync.router.add_post("/switch_character", switch_character)
    ...
    logger.info('情感陪护数字人系统: http://<serverip>:'+str(opt.listenport)+'/emotion_companion.html')
