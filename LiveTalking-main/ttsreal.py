...
class BaiduTTS(BaseTTS):
    ...
    def __init__(self, opt, parent):
        super().__init__(opt, parent)
        # 从环境变量或 companion_llm 获取配置
        import companion_llm
        char_config = companion_llm.get_character_config()
        self.voice = char_config.get("voice", "CAP_4194")  # 默认度嫣然
        self.app_id = companion_llm.BAIDU_TTS_APP_ID
        self.api_key = companion_llm.BAIDU_TTS_API_KEY
        self.secret_key = companion_llm.BAIDU_TTS_SECRET_KEY
    ...
    def txt_to_audio(self, msg: tuple[str, dict]):
        text, textevent = msg
        self.stream_tts(
            self.baidu_voice(text),
            msg
        )
    
    def baidu_voice(self, text) -> Iterator[bytes]:
        """调用百度语音合成 API"""
        start = time.perf_counter()
        
        access_token = self.get_access_token()
        if not access_token:
            logger.error("百度TTS: 无法获取access_token")
            return
        
        # 更新声线（根据当前角色）
        import companion_llm
        char_config = companion_llm.get_character_config()
        voice = char_config.get("voice", "CAP_4194")
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if "audio" in content_type:
                    ...
                    logger.info(f"百度TTS Time: {end-start}s, voice: {voice}")
                    yield res.content
                else:
                    # 返回错误信息
                    logger.error(f"百度TTS错误: {res.text}")
            else:
                logger.error(f"百度TTS请求失败: {res.status_code}")
        except Exception as e:
            logger.exception(f"百度TTS异常: {e}")
        ...
        for chunk in audio_stream:
            if chunk is not None and len(chunk) > 0:
                ...
                try:
                    ...
                    logger.exception(f"百度TTS音频处理失败: {e}")
...
class SiliconFlowTTS(BaseTTS):
    """SiliconFlow TTS - 支持 MOSS-TTSD 等模型"""
    
    def __init__(self, opt, parent):
        super().__init__(opt, parent)
        # 从 companion_llm 获取配置
        import companion_llm
        char_config = companion_llm.get_character_config()
        self.voice = char_config.get("voice", "alex")  # 默认 alex
        self.api_key = companion_llm.SILICONFLOW_TTS_API_KEY
        self.base_url = companion_llm.SILICONFLOW_TTS_BASE_URL
        self.model = companion_llm.SILICONFLOW_TTS_MODEL
        
    def txt_to_audio(self, msg: tuple[str, dict]):
        text, textevent = msg
        self.stream_tts(
            self.siliconflow_voice(text),
            msg
        )
    
    def siliconflow_voice(self, text) -> Iterator[bytes]:
        ...
        import companion_llm
        char_config = companion_llm.get_character_config()
        voice = char_config.get("voice", "alex")
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                logger.info(f"SiliconFlow TTS Time: {end-start}s, voice: {voice}")
                yield res.content
            else:
                logger.error(f"SiliconFlow TTS错误: {res.status_code} - {res.text}")
        except Exception as e:
            logger.exception(f"SiliconFlow TTS异常: {e}")
        ...
        for chunk in audio_stream:
            if chunk is not None and len(chunk) > 0:
                ...
                try:
                    ...
                    logger.exception(f"SiliconFlow TTS音频处理失败: {e}")
